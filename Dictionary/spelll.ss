...
(define verifyWord
  (lambda (hashfunctionlist w bitvector)
    (cond ((null? hashfunctionlist) #t)((and (check (getProp (car hashfunctionlist) w) bitvector) (verifyWord (cdr hashfunctionlist) w bitvector)) #t)(else #f))
))
...
;;used to check if the boolean value objects are equal 
(define hasObject
...
(define gen-checker
  (lambda (hashfunctionlist dict)
    (lambda (w)
      ...
      (verifyWord hashfunctionlist w bitvector)
)))
